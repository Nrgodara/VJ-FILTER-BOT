...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ...
        await message.reply_text(text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href=https://t.me/moviesearch_grouptelegram>ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/moviesearch_grouptelegram")]]))
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"📂 [{get_size(file.file_size)}] ♡⃕ {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}", callback_data=f'{pre}#{file.file_id}'
                ),
            ]
            for file in files
        ]
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        try:
            # Extract the size limit in bytes from the callback data
            size_limit_bytes = int(query.data.split("_")[2])
            ...
            await query.message.edit(f"An error occurred during deletion: {str(e)}")
            return
        ...
        try:
            ...
            # Handle any errors that occur during the cancelation process
            await query.message.edit(f"An error occurred while canceling: {str(e)}")
            return
    ...
    # InlineKeyboardButton('ʀᴇɴᴀᴍᴇ', callback_data='r_txt'),   
       ...
       #  ], [ 
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Sᴏᴜʀᴄᴇ Cᴏᴅᴇ', user_id=1280494242)
        ],[
